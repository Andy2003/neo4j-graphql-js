...
function parseFulltextDirective({
    ...
    value.forEach((index) => {
        const names = value.filter((i) => index.indexName === i.indexName);
        if (names.length > 1) {
            throw new Error(
                `Node '${definition.name.value}' @fulltext index contains duplicate name '${index.indexName}'`
            );
        }
        ...
        index.fields.forEach((field) => {
            ...
            if (!foundField) {
                throw new Error(
                    `Node '${definition.name.value}' @fulltext index contains invalid index '${index.indexName}' cannot use find String or ID field '${field}'`
                );
            }
        });
    });
